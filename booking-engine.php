...
function show_widget() {
    include_once( WP_PLUGIN_DIR . '/wp-booking-engine-1/views/i18n/'. get_option('omnibees_idioma') .'.php' );
...
function load_scripts(){    
    ...
    wp_enqueue_script(
        ...
        plugins_url() . '/wp-booking-engine-1/views/'. get_option('omnibees_template') .'/js/flatpickr.min.js', 
        array('jquery')
    );
    wp_enqueue_script(
        'location-flatpickr-script',
        plugins_url() . '/wp-booking-engine-1/views/'. get_option('omnibees_template') .'/js/location/'. $local .'.js',
